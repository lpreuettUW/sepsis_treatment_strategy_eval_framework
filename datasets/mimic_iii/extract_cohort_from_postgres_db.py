...
This file is originally sourced and modified from: https://github.com/matthieukomorowski/AI_Clinician
Original Author (MATLAB Implementation):
    Matthieu Komorowski

Adapted from https://github.com/microsoft/mimic_sepsis/blob/main/preprocess.py (MIT License)
Authors (Python Implementation):
...
# parser.add_argument("-p", "--password", default='PASSWORD', help="User's password for MIMIC Database", type=str) # NOTE: postgres password is disabled on my system (see https://dba.stackexchange.com/questions/83164/postgresql-remove-password-requirement-for-user-postgres)
pargs = parser.parse_args()

# Initializing database connection
# conn = pg.connect("dbname='mimic_iii' user={0} host='localhost' options='--search_path=mimimciii'".format(pargs.username)) # password={1}".format(pargs.username, pargs.password))
# NOTE: we switched from psycopg2 to sqlalchemy to suppress the following warning: 'UserWarning: pandas only supports SQLAlchemy connectable (engine/connection) or database string URI or sqlite3 DBAPI2 connection. Other DBAPI2 objects are not tested. Please consider using SQLAlchemy.'
db_engine = sqlalchemy.create_engine(sqlalchemy.engine.URL.create(
    drivername='postgresql',
    username=f'{pargs.username}',
    password='', # f'{pargs.password}' # NOTE: postgres password is disabled on my system (see https://dba.stackexchange.com/questions/83164/postgresql-remove-password-requirement-for-user-postgres)
    host='localhost',
    port=5432,
    database='mimic_iii'
))

...
Adapted from https://github.com/acneyouth1996/RL-for-sepsis-continuous/blob/yong_v0/scripts/sepsis_cohort.py (MIT License)
Authors:
...
parser.add_argument('--out_dir', type=str, default='preprocessed_cohort', help='Directory to save processed files')
pargs = parser.parse_args()

if not os.path.isdir(pargs.out_dir):
    os.makedirs(pargs.out_dir)
...
print('Full ICU -- Number of patients: ', num_patients)
...
# Remove patients with < 8h of data (we need at least 2x4h = 8h to have a trajectory of a single timestep - we need the next action)
reformat4t = reformat4t[reformat4t['icustayid'].isin(reformat4t.loc[(reformat4t['bloc'] > 1), 'icustayid'])]
########################################################################
...
if pargs.save_intermediate:
    sepsis.to_csv(os.path.join(pargs.out_dir, 'new_sepsis_mimiciii.csv'), index=False)
...
if pargs.save_intermediate:
    MIMICtable.to_csv(os.path.join(pargs.out_dir, 'MIMICtable.csv'), index=False)
...
iv_fluid_actions = np.expand_dims(MIMICtable['input_4hourly'].values, 1)
vp_actions = np.expand_dims(MIMICtable['max_dose_vaso'].values, 1)
continuous_actions = np.hstack([iv_fluid_actions, vp_actions])
...
cont_ac_df          = pd.DataFrame(continuous_actions, columns=['iv_fluid', 'vasopressor'])
...
cont_ac_df['traj']  = (raw_data_df['bloc'] == 1).cumsum().values
...
for i in trajectories:
    bar.update()
    data['traj'][i] = {}
    data['traj'][i]['meta'] = meta_df[meta_df['traj']==i][meta_cols].values.T[:, :-1]
    data['traj'][i]['obs'] = ob_df[ob_df['traj'] == i][ob_cols].values.T[:, :-1]
    data['traj'][i]['actions'] = ac_df[ac_df['traj'] == i]['action'].values.astype(np.int32)[1:] # actions need to be shifted
    data['traj'][i]['cont_actions'] = cont_ac_df[cont_ac_df['traj'] == i].values[1:, :-1] # shift actions and cut out the traj column
...
col_names.append('a:cont_iv_fluid')
col_names.append('a:cont_vp')
...
for i in data['traj'].keys():
    ...
    for ctr in range(data['traj'][i]['actions'].shape[0]):
        ...
        all_data[-1].append(data['traj'][i]['cont_actions'][ctr][0])
        all_data[-1].append(data['traj'][i]['cont_actions'][ctr][1])
        all_data[-1].append(data['traj'][i]['rewards'][ctr])
df = pd.DataFrame(all_data, columns=col_names)
df.to_csv(os.path.join(pargs.out_dir, 'sepsis_final_data_withTimes_90_day_death_window.csv'), index=False)
...
if pargs.process_raw: # If we want to convert the MIMICraw data into trajectories, repeat the above code without normalizing
    ...
    for i in data['traj'].keys():
        ...
        for ctr in range(data['traj'][i]['actions'].shape[0]):
            ...
            all_data[-1].append(data['traj'][i]['rewards'][ctr])
    df = pd.DataFrame(all_data, columns=col_names)
    df.to_csv(os.path.join(pargs.out_dir,'sepsis_final_data_RAW_withTimes_90_day_death_window.csv'), index=False)
